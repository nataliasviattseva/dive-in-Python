...
[Стандартные типы в Python](https://docs.python.org/3/library/stdtypes.html)

[Туториал по коллекциям из документации](https://docs.python.org/3/tutorial/datastructures.html)
...
---
### ***Exercice de programmation:* Key-value хранилище**
...
```
1 $ storage.py --key key_name --val value
```

**Получение значения по ключу key_name:**

```
1 $ storage.py --key key_name
```
...
2 import tempfile
3 
4 storage_path = os.path.join(tempfile.gettempdir(), 'storage.data')
...
Пример работы:
```
1 $ python storage.py --key key_name --val value
2 $ python storage.py --key key_name
3 value
4
5 $ python storage.py --key multi_key --val value1
6 $ python storage.py --key multi_key --val value2
7 $ python storage.py --key multi_key
8 value1, value2
```

---
### ***Exercice de programmation:* Декоратор to_json**
